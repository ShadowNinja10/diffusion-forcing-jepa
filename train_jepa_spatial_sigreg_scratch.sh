#!/bin/bash -e
#SBATCH --job-name=jepa-spatial-sigreg-scratch
#SBATCH --gres=gpu:4
#SBATCH --nodes=1
...
#SBATCH --mem=60G
#SBATCH -t 16:00:00
#SBATCH --output=logs/jepa-spatial-sigreg-scratch_%j.out
#SBATCH --error=logs/jepa-spatial-sigreg-scratch_%j.err
...
srun python main.py \
  +name=jepa_spatial_sigreg \
  experiment=video_generation \
  dataset=minecraft \
  algorithm=dfot_video_jepa_spatial_sigreg \
  dataset_experiment=minecraft_jepa_spatial_sigreg \
  @DiT/B \
...
# wandb video format is set in utils/logging_utils.py (format="gif") to avoid deprecation warning
