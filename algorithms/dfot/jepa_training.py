"""
Standalone JEPA Training (no DiT co-training).

Inherits the dual-encoder BYOL architecture from DFoTVideoJEPA but removes
the DFoT diffusion backbone entirely, allowing focused iteration on the
JEPA recipe (encoder, predictor, decoder probe).

Diagnostics:
- Encoder drift monitoring via KL divergence on a fixed reference image.
- Collapse detection via batch variance and channel std.
- Both teacher-forced and autoregressive prediction visualisations.

Gradient paths:
  JEPA loss     -->  online encoder + predictor + action encoder
  Decoder loss  -->  online decoder only (encoder detached by default)
"""
...
class JEPATraining(DFoTVideoJEPA):
    """
    Standalone JEPA training without DiT.

    Inherits all JEPA machinery (dual encoder, predictor, EMA, decoder probe,
    loss functions, visualisation helpers) from DFoTVideoJEPA and strips out
    everything related to the diffusion backbone.
    """

    def __init__(self, cfg: DictConfig):
        super().__init__(cfg)
        # Parent (DFoTVideoJEPA) sets automatic_optimization = False for
        # its two-phase backward.  We only have one loss so automatic is fine.
    ...
    def _build_model(self):
        """Build VAE + JEPA components; skip diffusion backbone entirely."""
    ...
    def _log_jepa_predictions_autoregressive(
        ...
        """
        True autoregressive rollout: only s_0 and a_0..a_{T-2} are given;
        the predictor feeds its own predictions back through the full causal
        transformer history at every step.

        Layout: PNG grid with (T-1) rows x 2 cols = [predicted | gt].
        """
        import torchvision
        ...
        states_so_far = online_states[:, 0:1]
        pred_list = []
        for t in range(T - 1):
            acts_so_far = action_embeds[:, : t + 1]
            preds = self.predictor(states_so_far, acts_so_far)
            next_pred = preds[:, -1:]
            pred_list.append(next_pred)
            states_so_far = torch.cat(
                [states_so_far, next_pred], dim=1
            )

        states_pred = torch.cat(pred_list, dim=1)

        pred_latents = states_pred.reshape(
            B * (T - 1), *latent_shape
        )
        pred_recon = self._decode_online(pred_latents)
